...
CREATE TABLE IF NOT EXISTS public.virgue_admins (
  user_id uuid PRIMARY KEY REFERENCES neon_auth."user"(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.virgue_trial_grants
  ADD COLUMN IF NOT EXISTS granted_by uuid REFERENCES public.virgue_admins(user_id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS note text NOT NULL DEFAULT '' CHECK (char_length(note) <= 280);

CREATE INDEX IF NOT EXISTS virgue_trial_grants_granted_by_idx
  ON public.virgue_trial_grants (granted_by);
...
INSERT INTO public.virgue_admins (user_id)
SELECT id
FROM neon_auth."user"
